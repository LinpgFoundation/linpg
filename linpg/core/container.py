...
class SurfaceContainerWithScrollbar(AbstractImage):
    ...
    def get_scroll_bar_pos(self) -> str:
        if self.__mode is True:
            return "top" if self.__scroll_bar_pos is True else "bottom"
        else:
            return "left" if self.__scroll_bar_pos is True else "right"
    ...
    def remove(self, key:Union[str,int]) -> None:
        if not self.__mode:
            self.__total_height -= (self.items[key].get_height()+self.distance_between_item)
        else:
            self.__total_width -= (self.items[key].get_width()+self.distance_between_item)
        del self.items[key]
    #清除所有物品
    def clear(self) -> None:
        self.items.clear()
            ...
            for event in pygame_events:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 1:
                        ...
                        if not self.__is_holding_scroll_button and scroll_button_rect is not None and\
                            scroll_button_rect.x < mouse_pos[0] < scroll_button_rect.right and\
                                scroll_button_rect.y < mouse_pos[1] < scroll_button_rect.bottom:
    ...
    def draw(self, surface:pygame.Surface) -> None:
        #如果有背景图片，则画出
        if self.img != None: surface.blit(self.img,self.pos)
        #防止local坐标越界
        if not self.__mode:
        ...
        for item in self.items.values():
            if not self.__mode:
                ...
                if 0 <= abs_local_y < self._height:
                    ...
                    if new_height > item.get_height(): new_height = item.get_height()
                    new_width = item.get_width()
                    if new_width > self._width: new_width = self._width
                    surface.blit(item.subsurface(pygame.Rect(0,0,new_width,new_height)),(current_x,current_y))
                elif -(item.get_height()) <= abs_local_y < 0:
                    crop_height = -abs_local_y
                    new_height = item.get_height()-crop_height
                    if new_height > self._height: new_height = self._heigh
                    new_width = item.get_width()
                    if new_width > self._width: new_width = self._width
                    surface.blit(item.subsurface(pygame.Rect(0,crop_height,new_width,new_height)),(current_x,current_y+crop_height))
                ...
                if 0 <= abs_local_x < self._width:
                    ...
                    if new_width > item.get_width(): new_width = item.get_width()
                    surface.blit(item.subsurface(pygame.Rect(0,0,new_width,item.get_height())),(current_x,current_y))
                elif -(item.get_width()) <= abs_local_x < 0:
                    crop_width = -abs_local_x
                    new_width = item.get_width()-crop_width
                    if new_width > self._width: new_width = self._width
                    surface.blit(item.subsurface(pygame.Rect(crop_width,0,new_width,item.get_height())),(current_x+crop_width,current_y))
