...
class SpriteImage:
    ...
    def split_sprite_image(_image_path: str, output_directory: str, _data: dict[str, tuple[int, int, int, int, int]]) -> None:
        ...
        for key, value in _data.items():
            ...
            for i in range(value[4]):
                Images.save(
                    _sprite_image.subsurface((value[0] + i * value[2], value[1], value[2], value[3])),
                    os.path.join(_out_path, f"{key}_{i}.png"),
                )
    ...
    @staticmethod
    def __put_and_document(
        _pos: tuple[int, int], minimize_pixels: bool, value: dict, sprite_surface: ImageSurface
    ) -> list[int]:
        _rect: list[int]
        if not minimize_pixels:
    ...
    def generate(
        ...
        for _path in Files.natural_sort(glob(os.path.join(img_folder_path, "*"))):
            ...
            if os.path.isdir(_path):
                ...
                _cache[_name] = [
                    cls.__process_image(_imgPath, max_block_size, minimize_pixels)
                    for _imgPath in Files.natural_sort(glob(os.path.join(_path, "*")))
                ]
            ...
            for key, value in _cache.items():
                if isinstance(value, dict):
                    _out["animations"][key] = cls.__put_and_document(
                        (
                            (index % columns) * max_block_size[0] + additional_padding,
                            index // columns * max_block_size[1] + additional_padding,
                        ),
                        minimize_pixels,
                        value,
                        sprite_surface,
        ...
        # 同一动作必须同行，方便读取管理
            ...
            for key in sorted(_cache, key=lambda k: len(_cache[k]) if isinstance(_cache[k], list) else 1):
                ...
                if isinstance(value, dict):
                    _out["animations"][key] = cls.__put_and_document(
                        (
                            current_column * max_block_size[0] + additional_padding,
                            current_row * max_block_size[1] + additional_padding,
                        ),
                        minimize_pixels,
                        value,
                        sprite_surface,
                    ...
                    for i in range(1, len(value)):
                        ...
                        current_column += 1
            sprite_surface = sprite_surface.subsurface(
                (0, 0, sprite_surface.get_width(), (current_row + 1) * max_block_size[1])
            )
        # 保存sprite图
