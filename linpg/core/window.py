...
# 基于ImageSurface的内部窗口
class AbstractWindow(AdvancedAbstractImageSurface, metaclass=ABCMeta):
    ...
    # 角落是否被触碰
    def __is_corner_hovered(self, side1: Directions, side2: Directions | None = None) -> bool:
        if side2 is None:
    ...
    def display(self, _surface: ImageSurface, offSet: tuple[int, int] = ORIGIN) -> None:
        ...
        if not Controller.mouse.get_pressed_previously(0):
            abs_x = Controller.mouse.x - self.x - offSet[0]
            abs_y = Controller.mouse.y - self.y - offSet[1]
            if (
            ...
            # 如果鼠标按住bar
            if Controller.mouse.get_pressed(0) and True not in self.__rescale_directions:
                if Controller.mouse.is_in_rect(
                    self.x + offSet[0], self.y + offSet[1], self.get_width(), self._get_bar_height()
                ):
            ...
            if self.__mouse_hovered_offset_pos is not None:
                ...
                # 向左放大
                if self.__rescale_directions[Directions.LEFT] is True:
                    if Controller.mouse.x < self.right:
                        self.set_width(self.right - Controller.mouse.x)
                        self.set_left(Controller.mouse.x)
                    else:
                        self.__rescale_directions[Directions.LEFT] = False
                        self.__rescale_directions[Directions.RIGHT] = True
                # 向右放大
                if self.__rescale_directions[Directions.RIGHT] is True:
        ...
        if Surfaces.is_not_null(self._content_surface):
            # 计算坐标
            abs_pos_x: int = self.x + self._get_outline_thickness() + offSet[0]
            abs_pos_y: int = self.y + self._get_bar_height() + self._get_outline_thickness() + offSet[1]
            real_local_x: int = 0
            real_local_y: int = 0
            if self.local_x < 0:
            ...
            height_of_sub: int = Numbers.keep_int_in_range(
                ...
                min(
                    ...
                    self.get_height() - self._get_bar_height() - self._get_outline_thickness(),
                ),
            )
            # 展示内容
