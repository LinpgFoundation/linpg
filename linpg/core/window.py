...
from ..basic.specifications import Version
from .saves import *
...
class LinpgVersionChecker:
    def __init__(self, action: str, recommended_revision: int, recommended_patch: int, recommended_version: int = 3) -> None:
        if not Version.ensure_linpg_version(action, recommended_revision, recommended_patch, recommended_version):
            if not ConfirmationDialogBox.ask(
                Languages.get_text("Global", "warning"),
                Languages.get_text("LinpgVersionIncorrectMessage").format(
                    f"3.{recommended_revision}.{recommended_patch}", Version.get_current_version()
                ),
            ):
