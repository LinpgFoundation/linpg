...
class Message:
    ...
    def show(self):
        return messagebox(
            self.title, self.message, None, self.info, self.warn, self.error, self.buttons, self.return_button, self.escape_button
            )

#窗口
class RenderedWindow(Rect):
    def __init__(self, width:number, height:number, title:str, is_win_always_on_top:bool):
