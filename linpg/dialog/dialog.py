...
class DialogSystem(AbstractDialogSystem):
    ...
    def draw(self, surface: ImageSurface) -> None:
        ...
        if (
            ...
            and currentDialogContent["next_dialog_id"] is not None
            and currentDialogContent["next_dialog_id"]["type"] == "option"
        ):
            optionBox_y_base = (
                Display.get_height() * 3 / 4
                - (len(currentDialogContent["next_dialog_id"]["target"])) * 2 * Display.get_width() * 0.03
            ) / 4
            ...
            for i in range(len(currentDialogContent["next_dialog_id"]["target"])):
                ...
                if (
                    ...
                    if leftClick and not self._is_showing_history:
                        nextDialogId = currentDialogContent["next_dialog_id"]["target"][i]["id"]
                else:
        ...
        # 如果操作或自动播放系统告知需要更新
            ...
            if currentDialogContent["next_dialog_id"] is None:
                ...
                # 如果是需要播放过程动画
                    ...
                    cutscene(
                        surface,
                        os.path.join(self._dynamic_background_folder_path, currentDialogContent["next_dialog_id"]["target"]),
                    )
                # break被视为立刻退出，没有淡出动画
