...
class DialogBox:
    def __init__(self, fontSize:int_f):
        ...
        #对胡框图片
        self.dialoguebox:StaticImage = StaticImage(os.path.join(DIALOG_UI_PATH, "dialoguebox.png"), 0, 0)
    ...
    def update(self, txt:list, narrator:str, forceNotResizeDialoguebox:bool=False) -> None:
        ...
        if self.__dev_mode is True: self.play_all()
    def reset(self) -> None:
    ...
    def __draw_dialogbox(self,surface:ImageSurface) -> None:
        self.dialoguebox.set_pos(int(surface.get_width()*0.13), int(self.dialoguebox_y))
        self.dialoguebox.set_size(surface.get_width()*0.74, self.dialoguebox_height)
