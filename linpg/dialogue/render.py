...
class VisualNovelCharacterImageManager:
    ...
    def draw(cls, _surface: ImageSurface) -> None:
        ...
        if cls.__last_round_image_alpha > 0:
            ...
            cls.__x_offset_for_last_round = int(
                cls.__GET_WIDTH() / 4 - cls.__GET_WIDTH() / 4 * cls.__last_round_image_alpha / 2550
            )
        else:
        ...
        if cls.__this_round_image_alpha < 2550:
            ...
            cls.__x_offset_for_this_round = int(
                cls.__GET_WIDTH() / 4 * cls.__this_round_image_alpha / 2550 - cls.__GET_WIDTH() / 4
            )
        else:
        ...
        if len(cls.__previous_characters) == len(cls.__current_characters):
            ...
            if len(cls.__previous_characters) == 1 and len(cls.__current_characters) == 2:
                ...
                if cls.__previous_characters[0].equal(cls.__current_characters[0]):
                    ...
                    # 显示右边立绘
                    cls.__display_character(
                        cls.__current_characters[1], _surface.get_width() // 2, cls.__this_round_image_alpha // 10, _surface
                    )
                ...
                if cls.__previous_characters[0].equal(cls.__current_characters[0]):
                    if cls.__x_correction_offset_index < 100:
                        ...
                        # 显示左方立绘
                        cls.__display_character(
                            cls.__current_characters[0], current_x, cls.__this_round_image_alpha // 10, _surface
                        )
                    # 右边立绘消失
                    cls.__display_character(
                        cls.__previous_characters[1], _surface.get_width() // 2, cls.__last_round_image_alpha // 10, _surface
                    )
