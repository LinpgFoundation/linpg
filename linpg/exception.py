...
class Exceptions:
    # 错误报告存储的路径
    __CRASH_REPORTS_PATH: Final[str] = "logs"
    # 引擎启动时的时间戳
    __TIME_STAMP_WHEN_LINPG_STARTED: Final[str] = datetime.now().strftime("%Y%m%d")
    ...
    def __log(cls, msg: str) -> None:
        ...
        # 写入讯息
        with open(
            os.path.join(cls.__CRASH_REPORTS_PATH, f"crash_{cls.__TIME_STAMP_WHEN_LINPG_STARTED}.txt"), "a", encoding="utf-8"
        ) as f:
        ...
        match error_type_id:
            case 1:
                raise cls.FunctionIsNotImplemented(
                    f'A parent class requires you to implement "{info}" function before you can use it'
                )
            case 2:
