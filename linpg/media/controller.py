...
class Controller:
    ...
    class __JoystickController:
        ...
        def update(cls) -> None:
            ...
            # 当目前有手柄在连接
                ...
                if pygame.joystick.get_count() == 0:
                    cls.__input = None
                    Exceptions.inform("Lost connection with the joystick.")
                # 有新的手柄
                elif cls.__input.get_instance_id() != pygame.joystick.Joystick(0).get_instance_id():
                    cls.__input = pygame.joystick.Joystick(0)
                    Exceptions.inform("Joystick changed! New joystick is detected and initialized successfully.")
    ...
    def get_event(cls, event_type: str) -> bool:
        ...
        if _result is None:
            Exceptions.fatal(f'The event type "{event_type}" is not supported!')
        return _result
