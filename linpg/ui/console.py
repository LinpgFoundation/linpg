from ..basic.variables import Variables
...
class Console(SingleLineInputBox, Hidable, threading.Thread):
    ...
    def _check_key_down(self, event: Event) -> bool:
        ...
        match event.key:
            ...
            case Keys.RETURN:
                ...
                if len(self._text) > 0:
                    ...
                    Exceptions.inform("The input box is empty!")
                self.__THREADING_LOCK.release()
                return True
            # ESC，关闭
            case Keys.ESCAPE:
    ...
    def _check_command(self, command_blocks: list[str]) -> None:
        match command_blocks[0]:
            ...
            case "set":
                Settings.set(*command_blocks[1 : len(command_blocks) - 1], value=command_blocks[len(command_blocks) - 1])
            case "setgv":
                Variables.Global.set(
                    *command_blocks[1 : len(command_blocks) - 1],
                    value=command_blocks[len(command_blocks) - 1],
                    assumeKeyExists=False,
                )
            case "setpv":
                Variables.Persistent.set(
                    *command_blocks[1 : len(command_blocks) - 1], value=command_blocks[len(command_blocks) - 1]
                )
            case "getgv":
                if command_blocks[1] == "*":
                    for key in Variables.Global.keys():
                        self._txt_output.append(f"{key}: {Variables.Global.try_get(key, _deepcopy=False)}")
                else:
                    gv_keys: list = command_blocks[1 : len(command_blocks) - 1]
                    self._txt_output.append(f"{gv_keys}: {Variables.Global.try_get(*gv_keys, _deepcopy=False)}")
            case "getpv":
                if command_blocks[1] == "*":
                    for key in Variables.Persistent.keys():
                        self._txt_output.append(f"{key}: {Variables.Persistent.try_get(key, _deepcopy=False)}")
                else:
                    pv_keys: list = command_blocks[1 : len(command_blocks) - 1]
                    self._txt_output.append(f"{pv_keys}: {Variables.Persistent.try_get(*pv_keys, _deepcopy=False)}")
            case "dev":
                if len(command_blocks) < 2:
                    ...
                    match self.__boolean_conversion(command_blocks[1]):
                        ...
                        case _:
                            self._txt_output.append("Unknown status for dev command.")
            case "linpg":
                self._txt_output.append(f"Linpg Version: {Version.get_full()}")
            case "quit":
    ...
    def draw(self, _surface: ImageSurface) -> None:
        if self.is_hidden():
            if any(
                event.type == Events.KEY_DOWN and event.unicode == self._COMMAND_INDICATOR for event in Controller.get_events()
            ):
        ...
        for event in Controller.get_events():
            match event.type:
                ...
                case Events.KEY_DOWN:
                    ...
                    elif event.key == Keys.BACKQUOTE or event.key == Keys.ESCAPE:
