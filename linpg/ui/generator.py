...
class UiGenerator:
    ...
    # 检测尺寸是否合法
    def __make_sure_size(
        self, item: dict, key: str, value_in_case_percentage: int
    ) -> None:
        if key not in item:
            EXCEPTION.fatal(
            ...
            if isinstance(item[key], str) and item[key].endswith("%"):
                try:
                    item[key] = int(
                        convert_percentage(item[key]) * value_in_case_percentage
                    )
                except Exception:
                    EXCEPTION.fatal(
                        ...
                        if "name" in item
                        else 'Cannot convert "{}" because it is not a valid percentage.'.format(
                            item[key]
                        )
                    )
            else:
                try:
                    ...
                    EXCEPTION.fatal(
                        'The "{0}" for "{1}" needs to an interger instead of "{2}".'.format(
                            key, item["name"], item[key]
                        )
                        if "name" in item
                        else 'The "{0}" needs to an interger instead of "{1}".'.format(
                            key, item[key]
                        )
                    )
            ...
            if text[text_index] == "{":
                ...
                if find_close_bracket is True:
                    ...
                    final_text_list.append(
                        Lang.get_text_by_keys(
                            (b.strip() for b in text[text_index + 1 : a].split(","))
                        )
                    )
                    text_index = a
                else:
    ...
    # 生成UI
    def generate(
        self, data: dict, max_width: int = -1, max_height: int = -1
    ) -> GameObject2d:
        ...
        if (
            ...
            if data["type"] == "text":
                ...
                item_t = DropDownSingleChoiceList(
                    data["src"], 0, 0, data["font_size"], data["color"]
                )
        else:
            ...
            if data["type"] == "container":
                # 转换坐标
                self.__make_sure_pos(
                    data, "x", int((max_width - data["width"]) / 2), max_width
                )
                self.__make_sure_pos(
                    data, "y", int((max_height - data["height"]) / 2), max_height
                )
                if "src" not in data:
                    data["src"] = None
                # 生成容器
                container_t = GameObjectsContainer(
                    data["src"], data["x"], data["y"], data["width"], data["height"]
                )
                # 加载数据
                if "hidden" in data:
                ...
                if "items" in data:
                    for each_item in data["items"]:
                        item_r = self.generate(
                            each_item, container_t.get_width(), container_t.get_height()
                        )
                        if item_r.tag != "":
                            container_t.set(item_r.tag, item_r)
                        else:
                            container_t.set(
                                "item{}".format(container_t.item_num), item_r
                            )
                return container_t
                ...
                try:
                    ...
                    EXCEPTION.fatal(
                        "You need to set a indicator for progress_bar_adjuster!"
                    )
                # 确认按钮有长宽
        ...
        # 转换坐标
        self.__make_sure_pos(
            data, "x", int((max_width - item_t.get_width()) / 2), max_width
        )
        self.__make_sure_pos(
            data, "y", int((max_height - item_t.get_height()) / 2), max_height
        )
