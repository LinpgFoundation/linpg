...
class UiGenerator:
    ...
    # 加载自定义的ui数据（如果存在）
    for key, value in Configurations.load(os.path.join("Data", "ui.json")).items():
    ...
    def __try_convert_string_to_number(cls, value: str, value_in_case_percentage: int, custom_values: dict) -> int:
        ...
        if value.endswith("%"):
            try:
                ...
                Exceptions.fatal(f'Cannot convert "{value}" because it is not a valid percentage.')
        # 如果是需要从lookup表里寻找的参数
            ...
            if isinstance(the_value, str):
                ...
                try:
                    ...
                    Exceptions.fatal(f'Cannot convert string "{value}".')
        # 尝试将
        else:
            try:
                return int(value)
    ...
    def __convert_number(cls, item: dict, key: str, value_in_case_percentage: int, custom_values: dict) -> int:
        if key not in item:
            Exceptions.fatal(
                f'You have to set "{key}" for "{item["name"]}".' if "name" in item else f'You have to set "{key}".'
            )
        ...
        if key not in item:
            ...
            if item[key] == "center":
                ...
                try:
                    ...
                    Exceptions.fatal(f"Valid value for {key}: {item[key]}.")
        else:
            return int(item[key])
    ...
    def __load_text(text: str) -> str:
                ...
                if text[text_index] == "{":
                    ...
                    if find_close_bracket is True:
                        ...
                        final_text_list.append(
                            Languages.get_text(*tuple(b.strip() for b in text[text_index + 1 : a].split(".")))
                        )
                        text_index = a
                    else:
                        Exceptions.fatal(f"Cannot find close bracket for text: {text}")
                else:
        ...
        # 加载自定义参数
            ...
            if _key != "NULL":
                return Specifications.get_str(*(b.strip() for b in _key.split(".")))
        return text
    ...
    def __generate(cls, data: dict, custom_values: dict, max_width: int = -1, max_height: int = -1) -> GameObject2d:
        ...
        if data["type"] == "container":
            ...
            if (
                data["type"] == "text"
                or data["type"] == "text&resize_when_hovered"
                ...
                if isinstance(data["type"], str) and data["type"].startswith("text"):
                    ...
                    if len(_info) < 2:
                        ...
                        item_t = ResizeWhenHoveredTextSurface(
                            data["src"], 0, 0, font_size, font_size * 3 / 2, data["color"], data["bold"], data["italic"]
                        )
                    else:
                        Exceptions.fatal(f'Unrecognized text format "{_info[1]}"')
                ...
                if data["type"] == "button":
                    ...
                    Exceptions.fatal("Current type is not supported")
            # 如果有名字，则以tag的形式进行标注
            ...
            if "visibility" in data:
                if isinstance(item_t, Hidable):
                    ...
                    Exceptions.fatal("This is not a subtype of Hidable!")
            # 设置坐标
            item_t.set_pos(
    ...
    def __get_data_in_dict(cls, data: str | dict) -> dict:
        if isinstance(data, str):
            ...
            if result is None:
                Exceptions.fatal(f'The ui called "{data}" does not exist!')
    ...
    def generate_container(cls, data: str | dict, custom_values: dict = {}) -> GameObjectsDictContainer:
        ...
        if data_dict["type"] != "container":
            Exceptions.fatal(f'The target has to be a container, not "{data_dict["type"]}".')
