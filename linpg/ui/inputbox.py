...
class SingleLineInputBox(AbstractInputBox):
    ...
    def _add_text(self, _content: str) -> None:
        if len(_content) > 0:
            ...
            Exceptions.inform("The value of event.unicode is empty!")
    ...
    def _remove_char(self, action: Locations) -> None:
        if action is Locations.BEGINNING:
            ...
            Exceptions.fatal("Action has to be either 'ahead' or 'behind'!")
        self._reset_inputbox_width()
...
class MultipleLinesInputBox(AbstractInputBox):
    def __init__(self, x: int_f, y: int_f, font_size: int, txt_color: color_liked, default_width: int = 150) -> None:
        ...
        if _SPEECH_RECOGNITION_ENABLED and len(sr.Microphone.list_working_microphones()) > 0:
            ...
            self.__start_dictating.set_text(ButtonComponent.text(Languages.get_text("Editor", "dictate"), font_size // 3))
    ...
    def _add_text(self, _content: str) -> None:
        if len(_content) > 0:
            if "\n" not in _content:
                self._text[self.__lineId] = (
                    self._text[self.__lineId][: self._holder_index] + _content + self._text[self.__lineId][self._holder_index :]
                )
                self._holder_index += len(_content)
                self._reset_inputbox_width()
            else:
    ...
    def _remove_char(self, action: Locations) -> None:
        if action is Locations.BEGINNING:
            if self._holder_index > 0:
                self._text[self.__lineId] = (
                    self._text[self.__lineId][: self._holder_index - 1] + self._text[self.__lineId][self._holder_index :]
                )
                self._holder_index -= 1
            ...
            if self._holder_index < len(self._text[self.__lineId]):
                self._text[self.__lineId] = (
                    self._text[self.__lineId][: self._holder_index] + self._text[self.__lineId][self._holder_index + 1 :]
                )
    ...
    def __dictate(self) -> None:
        ...
        try:
            ...
            Exceptions.warn("No speaker detected!")
        # try process audio
        if _audio is not None:
            # Recognize speech using Speech API
            try:
                self._add_text(recognizer.recognize_google(_audio, language=Languages.get_current_language_tag()))
            except sr.UnknownValueError:
                Exceptions.inform("Speech API could not understand the audio")
            except sr.RequestError as e:
                Exceptions.inform(f"Could not request results from Speech API; {e}")
    ...
    def display(self, _surface: ImageSurface, offSet: tuple[int, int] = ORIGIN) -> None:
        ...
        if self._text is not None:
            for i in range(len(self._text)):
                # 画出文字
                _surface.blit(
                    self._FONT.render(self._text[i], self._text_color),
                    (abs_pos[0] + self._FONT.size // 4, abs_pos[1] + i * self._default_height),
                )
        # 如果输入模式被激活
        if self._active:
            # 画出输入框
                ...
                if self._display_holder_timer.get_status():
                    _surface.blit(
                        ...
                        (
                            abs_pos[0]
                            + self._FONT.size // 10
                            + self._FONT.estimate_text_width(self._text[self.__lineId][: self._holder_index]),
