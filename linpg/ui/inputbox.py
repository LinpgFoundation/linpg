...
class SingleLineInputBox(AbstractInputBox):
    def __init__(
        ...
        self._padding: int = int(
            (self.input_box.height - self._holder.get_height()) / 2
        )
    ...
    def _add_char(self, char: str) -> None:
        if len(char) > 0:
            self._text = (
                self._text[: self.holderIndex] + char + self._text[self.holderIndex :]
            )
    ...
    def _remove_char(self, action: str) -> None:
        if action == "ahead":
            if self.holderIndex > 0:
                self._text = (
                    self._text[: self.holderIndex - 1] + self._text[self.holderIndex :]
                )
                self.holderIndex -= 1
        elif action == "behind":
            if self.holderIndex < len(self._text):
                self._text = (
                    self._text[: self.holderIndex] + self._text[self.holderIndex + 1 :]
                )
        else:
    ...
    def _reset_holderIndex(self, mouse_x: int) -> None:
        ...
        for i in range(len(self._text)):
            new_width = (
                self.FONT.estimate_text_width(self._text[:i]) + self.FONT.size * 0.25
            )
    ...
    def _reset_inputbox_width(self) -> None:
        if self._text is not None and len(self._text) > 0:
            self.input_box.set_width(
                max(
                    self.default_width,
                    self.FONT.estimate_text_width(self._text) + self.FONT.size * 0.6,
                )
            )
        else:
    ...
    def _draw_content(self, surface: ImageSurface, with_holder: bool = True) -> None:
        if self._text is not None and len(self._text) > 0:
            ...
            surface.blit(
                font_t,
                (
                    self.x + self._padding,
                    self.y + int((self.input_box.height - font_t.get_height()) / 2),
                ),
            )
            ...
            if int(time.time() % 2) == 0 or len(Controller.events) > 0:
                surface.blit(
                    ...
                    (
                        self.x
                        + self._padding
                        + self.FONT.estimate_text_width(self._text[: self.holderIndex]),
...
class MultipleLinesInputBox(AbstractInputBox):
    ...
    def set_text(self, new_txt: Union[tuple, list] = None) -> None:
        if new_txt is None or len(self._text) == 0:
            ...
            EXCEPTION.fatal(
                "The new_txt for MultipleLinesInputBox.set_text() must be a list!"
            )
    ...
    def _remove_char(self, action: str) -> None:
        if action == "ahead":
            if self.holderIndex > 0:
                self._text[self.lineId] = (
                    self._text[self.lineId][: self.holderIndex - 1]
                    + self._text[self.lineId][self.holderIndex :]
                )
                self.holderIndex -= 1
            elif self.lineId > 0:
    ...
    def _reset_holderIndex(self, mouse_x: int, mouse_y: int) -> None:
        ...
        for i in range(len(self._text[self.lineId])):
            new_width = (
                self.FONT.estimate_text_width(self._text[self.lineId][:i])
                + self.FONT.size * 0.25
            )
            if new_width > local_x:
                break
            ...
            if self.active:
                if event.type == Key.DOWN:
                    if event.key == Key.BACKSPACE:
                        ...
                        self.holderIndex -= 1
                    elif event.key == Key.ARROW_RIGHT and self.holderIndex < len(
                        self._text[self.lineId]
                    ):
                        self.holderIndex += 1
                        ...
                        and self.lineId < len(self._text) - 1
                    ):
                        self.lineId += 1
                        if self.holderIndex > len(self._text[self.lineId]) - 1:
                            self.holderIndex = len(self._text[self.lineId]) - 1
                    ...
                    # ESC，关闭
                        ...
                        if self.holderIndex == len(self._text[self.lineId]):
                            ...
                            self._text[self.lineId] = self._text[self.lineId][
                                : self.holderIndex
                            ]
                        self.lineId += 1
                        self.holderIndex = 0
...
class Console(SingleLineInputBox):
    def __init__(
        self, x: int_f, y: int_f, font_size: int = 32, default_width: int = 150
    ):
        ...
        # 回车
            ...
            if len(self._text) > 0:
                if self._text.startswith(self.command_indicator):
                    self._check_command(
                        self._text[len(self.command_indicator) :].split()
                    )
                else:
    ...
    def _check_command(self, conditions: list) -> None:
        if conditions[0] == "cheat":
            ...
            conditions[0] == "linpg" and len(conditions) > 1 and conditions[1] == "info"
        ):
        ...
        if self.hidden is True:
            ...
            for i in range(len(self._txt_output)):
                screen.blit(
                    ...
                    (
                        self.x + self.FONT.size * 0.25,
                        self.y - (len(self._txt_output) - i) * self.FONT.size * 1.5,
                    ),
                )
            # 画出输入框
            draw_rect(screen, self.color, self.input_box, 2)
