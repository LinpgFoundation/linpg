...
class OptionMenu(AbstractInternalMenu):
    ...
    def initialize(self) -> None:
        ...
        for lang_choice in Languages.get_available_languages():
            lang_drop_down.set(Languages.get_language_id(lang_choice), lang_choice)
        lang_drop_down.set_selected_item(Settings.get_language())
    ...
    def draw(self, _surface: ImageSurface) -> None:
        ...
        if self.is_visible():
            ...
            super().draw(_surface)
            # 如果需要更新语言
            if lang_drop_down.get_selected_item() != Settings.get_language() and lang_drop_down.get_selected_item() != "":
                # 更新语言并保存新的参数到本地
                Settings.set("Language", value=lang_drop_down.get_selected_item())
                Settings.save()
                Languages.reload()
            ...
            if self._CONTENT.item_being_hovered is not None and not lang_drop_down.is_hovered():
                ...
                if self._CONTENT.item_being_hovered == "global_sound_volume":
                    ...
                    if item_percentage_t != int(Settings.get("Sound", "global_value")):
                ...
                # 如果碰到背景音乐音量条
                    ...
                    if item_percentage_t != int(Settings.get("Sound", "background_music")):
                ...
                # 如果碰到音效的音量条
                    ...
                    if item_percentage_t != int(Settings.get("Sound", "effects")):
            ...
            if Controller.get_event("back") or Controller.get_event("hard_confirm"):
                ...
                for _y in range(self.row):
                    for _x in range(self.colum):
                        _rect.set_pos(
                            colum_padding + (colum_padding + rect_width) * _x, row_padding + (row_padding + rect_height) * _y
                        )
                ...
                if self.__select_progress_menu.is_visible():
                    ...
                    if self.__select_progress_menu.get_selected_slot() >= 0:
                        ...
                        if self.__save_or_load is True:
                            Saves.save(
                                self._get_data_need_to_save(), _screenshot, self.__select_progress_menu.get_selected_slot()
                            )
