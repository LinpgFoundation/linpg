...
class OptionMenu(AbstractInternalMenu):
    ...
    def initialize(self) -> None:
        ...
        if self._CONTENT is None:
            Exceptions.fatal("The ui has not been correctly initialized.")
        lang_drop_down: DropDownList = self._CONTENT.get("lang_drop_down")
        for lang_choice in Languages.get_available_languages():
            lang_drop_down.set(lang_choice, lang_choice)
        lang_drop_down.set_selected_item(Languages.get_current_language())
    ...
    def draw(self, _surface: ImageSurface) -> None:
        ...
        lang_drop_down: DropDownList = self._CONTENT.get("lang_drop_down")
        # 更新百分比
        self._CONTENT.get("global_sound_volume").set_percentage(Settings.get("Sound", "global_value") / 100)
        self._CONTENT.get("background_music_sound_volume").set_percentage(Settings.get("Sound", "background_music") / 100)
        self._CONTENT.get("effects_sound_volume").set_percentage(Settings.get("Sound", "effects") / 100)
        self._CONTENT.get("environment_sound_volume").set_percentage(Settings.get("Sound", "environment") / 100)
        ...
        if not self._CONTENT.is_background_init():
            ...
            Draw.rect(
                _surface,
                Colors.WHITE,
                Rectangles.add(self._CONTENT.get_rect(), (_padding, _padding, _padding * -2, _padding * -2)).get_rect(),
        ...
        super().draw(_surface)
        # 如果需要更新语言
        if lang_drop_down.get_selected_item() != Languages.get_current_language() and lang_drop_down.get_selected_item() != "":
            # 更新语言并保存新的参数到本地
            Settings.set("Language", value=Languages.get_language_id(lang_drop_down.get_selected_item()))
            Settings.save()
            Languages.reload()
        ...
        if self._CONTENT.item_being_hovered is not None and not lang_drop_down.is_hovered():
            ...
            match self._CONTENT.item_being_hovered:
                ...
                case "global_sound_volume":
                    ...
                    if item_percentage_t != Settings.get_int("Sound", "global_value"):
                        Settings.set("Sound", "global_value", value=item_percentage_t)
                ...
                case "background_music_sound_volume":
                    ...
                    if item_percentage_t != Settings.get_int("Sound", "background_music"):
                        Settings.set("Sound", "background_music", value=item_percentage_t)
                ...
                case "effects_sound_volume":
                    ...
                    if item_percentage_t != Settings.get_int("Sound", "effects"):
                        Settings.set("Sound", "effects", value=item_percentage_t)
                        self.need_update["volume"] = True
                # 如果碰到环境声的音量条
                case "environment_sound_volume":
                    item_percentage_t = int(self._CONTENT.get("environment_sound_volume").percentage * 100)
                    if item_percentage_t != Settings.get_int("Sound", "environment"):
...
class SaveOrLoadSelectedProgressMenu(Hidable):
    ...
    def __process_page_switching(self, _surface: ImageSurface) -> None:
        ...
        button_rect: Rectangle = Rectangle(
            padding, center_y - triangle_size - padding, triangle_size + padding * 2, padding * 2 + triangle_size * 2
        )
    ...
    def draw(self, _surface: ImageSurface) -> None:
        ...
        if Controller.get_event("back") or Controller.get_event("hard_confirm"):
            ...
            rect_width: int = _surface.get_width() // (self.column + 1)
            column_padding: int = rect_width // (self.column + 1)
            ...
            # 渲染页码
            pageIdText: ImageSurface = Fonts.render(f"- {self.__page_id} -", Colors.WHITE, row_padding // 2)
            _surface.blit(
                pageIdText,
                (
                    (_surface.get_width() - pageIdText.get_width()) // 2,
                    _surface.get_height() - row_padding + (row_padding - pageIdText.get_height()) // 2,
                ),
            )
            # 渲染存档信息
            for _y in range(self.row):
                for _x in range(self.column):
                    _rect.set_pos(
                        column_padding + (column_padding + rect_width) * _x, row_padding + (row_padding + rect_height) * _y
                    )
                    _slotId: int = (self.__page_id - 1) * self.column * self.column + _y * self.column + _x
                    ...
                    if _file is not None:
                        ...
                        _surface.blit(
                            Images.smoothly_resize_and_crop_to_fit(_file.screenshot, (_rect.get_width(), _img_height)),
                            _rect.get_pos(),
                        )
                        _createdAt: ImageSurface = Fonts.render(
                            f"{_file.createdAt} - Chapter {_file.data.get('chapter_id')}",
                            Colors.WHITE,
                            (_rect.get_height() - _img_height) // 2,
                        )
                        _surface.blit(
                            _createdAt,
                            (
...
class PauseMenuModuleForGameSystem(AbstractInternalMenu):
    ...
    @abstractmethod
    def stop(self) -> None:
        Exceptions.fatal("stop()", 1)

    # 更新音量（子类需实现）
    @abstractmethod
    def _update_sound_volume(self) -> None:
        Exceptions.fatal("_update_sound_volume()", 1)

    # 更新语言（子类需实现）
    @abstractmethod
    def update_language(self) -> None:
        Exceptions.fatal("update_language()", 1)
