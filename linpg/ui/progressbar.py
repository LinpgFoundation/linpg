...
class ProgressBar(AbstractProgressBar):
    ...
    def display(self, surface: ImageSurface, offSet: pos_liked = Pos.ORIGIN) -> None:
        if not self.hidden:
            draw_rect(surface, self.color, new_rect(Pos.add(self.pos, offSet), (int(self._width*self.percentage), self._height)))
    ...
    def display(self, surface:ImageSurface, offSet:pos_liked = Pos.ORIGIN) -> None:
        if not self.hidden:
            ...
            if self._mode:
                ...
                surface.blit(
                    value_font,
                    Pos.int(Pos.add(abs_pos, (self._width+self.__indicator.width*0.7, (self._height-value_font.get_height())/2)))
                    )
            else:
                x,y = Pos.int(Pos.add(
                ...
                surface.blit(
                    value_font,
                    Pos.int(Pos.add(abs_pos, ((self._width-value_font.get_width())/2, self._height+self.__indicator.height*0.7)))
                    )
