...
class AbstractScrollbarsSurface(SurfaceWithLocalPos):
    ...
    def _get_right_scroll_bar_rect(self, off_set_x: number, off_set_y: number) -> Rect:
        return (
            Rect(
                int(self.abs_x + self.get_width() - self._button_tickness + off_set_x),
    ...
    def _get_bottom_scroll_bar_rect(self, off_set_x: number, off_set_y: number) -> Rect:
        return (
            Rect(
                ...
                int(self.abs_y + self.get_height() - self._button_tickness + off_set_y),
                self.get_width(),
                self._button_tickness,
            )
    ...
    def display_scrollbar(self, surface: ImageSurface, off_set: tuple = ORIGIN) -> None:
        ...
        if self.is_hovered(off_set):
            if Controller.mouse.get_pressed(0):
                if self.get_surface_height() > self.get_height():
                    scroll_bar_rect = self._get_right_scroll_bar_rect(off_set[0], off_set[1])
                    if scroll_bar_rect.is_hovered():
                        scroll_button_rect = self._get_right_scroll_button_rect(off_set[0], off_set[1])
                ...
                if self.get_surface_width() > self.get_width():
                    scroll_bar_rect = self._get_bottom_scroll_bar_rect(off_set[0], off_set[1])
                    if scroll_bar_rect.is_hovered():
                        scroll_button_rect = self._get_bottom_scroll_button_rect(off_set[0], off_set[1])
                        if scroll_button_rect.is_hovered():
        ...
        # 画出滚动条
        if (scroll_button_rect := self._get_scroll_button_rect(off_set[0], off_set[1])) is not NULL_RECT:
            draw_rect(surface, Color.WHITE, scroll_button_rect)
        if (scroll_bar_rect := self._get_scroll_bar_rect(off_set[0], off_set[1])) is not NULL_RECT:
...
class AbstractSurfaceWithScrollbar(AbstractScrollbarsSurface):
    ...
    def display_scrollbar(self, surface: ImageSurface, off_set: tuple = ORIGIN) -> None:
        # 获取鼠标坐标
        if self.is_hovered(off_set):
            if (not self._mode and self.get_surface_height() > self.get_height()) or (
                self._mode is True and self.get_surface_width() > self.get_width()
            ):
                # 查看与鼠标有关的事件
                if Controller.mouse.get_pressed(0):
                    scroll_bar_rect = self._get_scroll_bar_rect(off_set[0], off_set[1])
                    if scroll_bar_rect is not NULL_RECT and scroll_bar_rect.is_hovered():
                        scroll_button_rect = self._get_scroll_button_rect(off_set[0], off_set[1])
                        # 根据按钮位置调整本地坐标
                        if scroll_button_rect.is_hovered():
